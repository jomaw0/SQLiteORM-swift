...
public extension ORM {
    ...
    func query<T: ORMTable>(_ type: T.Type) async -> Query<T> {
    ...
    func execute<T: ORMTable>(_ query: Query<T>) async -> ORMResult<[T]> {
        await query.fetch()
    }
    ...
    func executeFirst<T: ORMTable>(_ query: Query<T>) async -> ORMResult<T?> {
    ...
    func executeCount<T: ORMTable>(_ query: Query<T>) async -> ORMResult<Int> {
        await query.count()
    }
}