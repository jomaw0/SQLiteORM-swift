...
public struct Query<T: ORMTable>: Sendable {
    ...
    public func join(_ table: String, on condition: String) -> Query {
        var query = self
        query.joins.append(ORMJoinClause(type: .inner, table: table, condition: condition))
        return query
    }
    ...
    public func leftJoin(_ table: String, on condition: String) -> Query {
        var query = self
        query.joins.append(ORMJoinClause(type: .left, table: table, condition: condition))
        return query
    }
    ...
    /// Convert to legacy ORMQueryBuilder for compatibility
    private func toQueryBuilder() -> ORMQueryBuilder<T> {
        var builder = ORMQueryBuilder<T>()
    ...
    /// Apply a predicate to a ORMQueryBuilder
    private func applyPredicateToBuilder(_ builder: ORMQueryBuilder<T>, predicate: Predicate) -> ORMQueryBuilder<T> {
        switch predicate {
